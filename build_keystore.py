...
window.AIKeystore = {
  ...
  getKey: async function(password) {
    ...
    if (key) AIKeystore._sessionKey = key;
    return key;
  },
      ...
      '<div style="background:#0d1117;border:1px solid #2a3b48;border-radius:14px;' +
        ...
        '<div style="padding:24px;">' +
          ...
          '<div id="ks-error" style="display:none;background:#1a0808;border:1px solid #3a1a1a;' +
            'border-radius:6px;padding:8px 12px;color:#e05252;font-size:12px;margin-bottom:12px;">',
          '</div>' +
  ...
  initSession: function() {
    ...
    /* Якщо є ключ — просимо пароль */
    console.log('[Keystore] Key found, waiting for unlock...');
    /* Не показуємо автоматично — чекаємо першого запиту AI */
  },
...
with open('ai-agent/ai-agent-keystore.js', 'w', encoding='utf-8') as f:
    f.write(KEYSTORE_JS)
print('OK: ai-agent-keystore.js ✅')

# ── Оновлюємо ai-agent-core.js — використовуємо Keystore ──
with open('ai-agent/ai-agent-core.js', 'r', encoding='utf-8') as f:
    core = f.read()

# Замінюємо функцію send — додаємо отримання ключа
old_send = """  /* ── Головна функція: надіслати повідомлення ── */
...
new_send = """  /* ── Головна функція: надіслати повідомлення ── */
AIAgent.send = function(userMessage, options) {
  options = options || {};
  var includeContext = options.includeContext !== false;

  /* Отримуємо API ключ */
  return AIKeystore.getKeyForRequest()
    .then(function(apiKey) {
      if (!apiKey) return Promise.reject('API ключ не знайдено');
      return AIAgent.getRouterContext()
        .then(function(context) {
          var systemContent = AIAgent.config.systemPrompt;
          if (includeContext && context) {
          ...
          return fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
              model:       AIAgent.config.model,
              max_tokens:  AIAgent.config.maxTokens,
              temperature: AIAgent.config.temperature,
              messages:    messages,
            })
          })
          ...
          .then(function(d) {
            if (d.error) return Promise.reject(d.error.message || 'Groq error');
...
if old_send in core:
    ...
    print('OK: AIAgent.send оновлено ✅')
else:
    print('WARN: send не знайдено точно')

with open('ai-agent/ai-agent-core.js', 'w', encoding='utf-8') as f:
    f.write(core)

# ── Оновлюємо ai-agent-ui.js — додаємо кнопку 🔑 ──
with open('ai-agent/ai-agent-ui.js', 'r', encoding='utf-8') as f:
    ui = f.read()

old_key_btn = """        '<button onclick="window.AIAgentUI.runSecurityAudit()" title="Аудит безпеки"',
...
new_key_btn = """        '<button onclick="window.AIKeystore.showModal()" title="API Ключ"',
...
if old_key_btn in ui:
    ui = ui.replace(old_key_btn, new_key_btn)
    print('OK: кнопка 🔑 в sidebar ✅')
else:
    print('WARN: кнопка не знайдена')

with open('ai-agent/ai-agent-ui.js', 'w', encoding='utf-8') as f:
    f.write(ui)

# ── Підключаємо keystore в index.html ──
with open('index.html', 'r', encoding='utf-8') as f:
    html = f.read()

old_script = '  <!-- AI Agent -->\n  <script src="ai-agent/ai-agent-core.js"></script>'
new_script = '  <!-- AI Agent -->\n  <script src="ai-agent/ai-agent-keystore.js"></script>\n  <script src="ai-agent/ai-agent-core.js"></script>'

if 'ai-agent-keystore.js' not in html:
    html = html.replace(old_script, new_script)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print('OK: keystore в index.html ✅')

# ── Перевірка ──
errors = 0
for fn in ['ai-agent/ai-agent-keystore.js', 'ai-agent/ai-agent-core.js', 'ai-agent/ai-agent-ui.js']:
