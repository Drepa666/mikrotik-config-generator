...
# ════ Записуємо ════
with open('rm-torch.js', 'w', encoding='utf-8') as f:
    f.write(TORCH)

size = os.path.getsize('rm-torch.js')
r2 = subprocess.run(['node','--check','rm-torch.js'], capture_output=True, text=True)
print(f'rm-torch.js: {"OK ✅" if r2.returncode==0 else "FAIL"} ({size:,}b)')
if r2.returncode != 0:
    print(r2.stderr[:200])
    exit(1)
...
INSERT = '<script src="rm-torch.js"></script>\n    ' + TARGET

if 'rm-torch.js' in html:
    print('OK: вже є в index.html')
elif TARGET in html:
    html = html.replace(TARGET, INSERT, 1)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print('OK: rm-torch.js додано в index.html')

# ════ Git ════
subprocess.run(['git','add','rm-torch.js','index.html'], capture_output=True)
subprocess.run(['git','commit','-m',
    'feat: rm-torch.js - real-time traffic monitor, charts, per-interface stats'],
    capture_output=True)
