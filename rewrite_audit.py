...
(function() {
  ...
  function addEntry(action, details, cat) {
    var l = load();
    l.unshift({ id:Date.now(), ts:new Date().toLocaleString('uk-UA'), action:action||'', details:details||'', cat:cat||'general' });
  ...
  function renderLog() {
    ...
    filtered.forEach(function(e) {
      var color = COLORS[e.cat]||'#4a6070';
      var icon  = ICONS[e.cat]||'\uD83D\uDCCB';
  ...
  document.getElementById('al-exp-csv').addEventListener('click', function() {
    ...
    load().forEach(function(e) { csv += [e.id,'"'+e.ts+'"','"'+e.action+'"','"'+(e.details||'')+'"',e.cat].join(',')+'\n'; });
    var a = document.createElement('a');
  ...
  fab.addEventListener('click', function() { modal.style.display='block'; renderLog(); });
...
with open('audit.js', 'w', encoding='utf-8', newline='\n') as f:
    f.write(JS)
print('OK: audit.js повністю перезаписано!')
...
if 'audit.js' not in c:
    ...
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(c)
    print('OK: audit.js підключено до index.html!')
else:
    print('OK: audit.js вже в index.html!')