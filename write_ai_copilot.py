...
TARGET = '<script src="rm-torch.js"></script>'
INSERT = '<script src="rm-ai-copilot.js"></script>\n    ' + TARGET

if 'rm-ai-copilot.js' in html:
    print('OK: вже є в index.html')
elif TARGET in html:
    html = html.replace(TARGET, INSERT, 1)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print('OK: rm-ai-copilot.js додано в index.html')

# ════ Git ════
subprocess.run(['git','add','rm-ai-copilot.js','index.html'], capture_output=True)
subprocess.run(['git','commit','-m',
    'feat: AI Copilot - Firewall analysis, security audit, chat, quick questions'],
    capture_output=True)
