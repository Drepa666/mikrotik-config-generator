...
# ════ КРОК 1: Записуємо в tempfile і перевіряємо ════
...
# ════ КРОК 2: Записуємо файл ════
with open('rm-datagrid.js', 'w', encoding='utf-8') as f:
    f.write(DATAGRID)

size = os.path.getsize('rm-datagrid.js')
r2 = subprocess.run(['node', '--check', 'rm-datagrid.js'],
                    capture_output=True, text=True)
print(f'rm-datagrid.js: {"OK" if r2.returncode==0 else "FAIL"} ({size:,}b)')
if r2.returncode != 0:
    print(r2.stderr[:200])
    exit(1)

# ════ КРОК 3: Додаємо в index.html ПЕРЕД router-manager-crud.js ════
with open('index.html', 'r', encoding='utf-8') as f:
    html = f.read()

TARGET = '<script src="router-manager-crud.js"></script>'
INSERT = '<script src="rm-datagrid.js"></script>\n    ' + TARGET

if 'rm-datagrid.js' in html:
    print('OK: rm-datagrid.js вже є в index.html')
elif TARGET in html:
    html = html.replace(TARGET, INSERT, 1)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print('OK: rm-datagrid.js додано в index.html')
else:
    print('WARN: router-manager-crud.js не знайдено в index.html')
    print('Додай вручну: <script src="rm-datagrid.js"></script>')

# ════ КРОК 4: Git ════
subprocess.run(['git', 'add', 'rm-datagrid.js', 'index.html'], capture_output=True)
subprocess.run(['git', 'commit', '-m',
    'feat: rm-datagrid.js - sort, multi-select, drag-drop, context menu, keyboard nav, resize'],
    capture_output=True)
