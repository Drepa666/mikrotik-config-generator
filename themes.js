...
function createFab() {
  ...
  fab.addEventListener('mouseenter', function() {
    fab.style.background = '#5fd0a5';
    fab.style.color = '#082018';
  });
  fab.addEventListener('mouseleave', function() {
    fab.style.background = 'rgba(15,23,32,.92)';
    fab.style.color = '#5fd0a5';
  });
  fab.addEventListener('click', function(e) {
    ...
    /* Позиціонуємо панель під кнопкою */
  ...
  /* Вставляємо в lang-switcher або поруч з ним */
  function insertNearLang() {
    var sw = document.getElementById('lang-switcher');
    if (sw) {
      /* Додаємо роздільник і кнопку всередину lang-switcher */
      var sep = document.createElement('div');
      sep.style.cssText = 'width:1px;background:#2a3b48;margin:2px 2px;';
      sw.appendChild(sep);
      sw.appendChild(fab);
      console.log('[themes] fab додано в lang-switcher');
      return true;
    }
    return false;
  }
  ...
  if (!insertNearLang()) {
    ...
    var t = setInterval(function() {
      tries++;
      if (insertNearLang() || tries > 30) clearInterval(t);
    }, 200);
  }
}
...
function initThemes() {
  ...
  if (false) { /* кнопка в lang-switcher */
    var tries = 0;
