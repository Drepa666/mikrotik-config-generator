...
# ════ КРОК 2: Записуємо ════
with open('rm-safemode.js', 'w', encoding='utf-8') as f:
    f.write(SAFEMODE)

size = os.path.getsize('rm-safemode.js')
r2 = subprocess.run(['node', '--check', 'rm-safemode.js'],
                    capture_output=True, text=True)
print(f'rm-safemode.js: {"OK" if r2.returncode==0 else "FAIL"} ({size:,}b)')
if r2.returncode != 0:
    print(r2.stderr[:200])
    exit(1)

# ════ КРОК 3: Додаємо в index.html ════
with open('index.html', 'r', encoding='utf-8') as f:
    html = f.read()

TARGET = '<script src="rm-datagrid.js"></script>'
INSERT = '<script src="rm-safemode.js"></script>\n    ' + TARGET

if 'rm-safemode.js' in html:
    print('OK: rm-safemode.js вже є в index.html')
elif TARGET in html:
    html = html.replace(TARGET, INSERT, 1)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print('OK: rm-safemode.js додано в index.html')
else:
    print('WARN: target не знайдено')

# ════ КРОК 4: Git ════
subprocess.run(['git', 'add', 'rm-safemode.js', 'index.html'], capture_output=True)
subprocess.run(['git', 'commit', '-m',
    'feat: rm-safemode.js - F9 toggle, 60s countdown, auto-revert, per-section toggle'],
    capture_output=True)
